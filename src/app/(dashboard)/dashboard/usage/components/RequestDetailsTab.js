          ...
          <table className="w-full min-w-[980px]">
            <thead>
              <tr className="border-b border-black/5 dark:border-white/5">
                ...
                <th className="text-left p-4 text-sm font-semibold text-text-main">Provider API Key</th>
              ...
              {loading ? (
                ...
                details.map((detail, index) => (
                  ...
                  >
                    ...
                    <td className="max-w-[180px] p-4 text-sm text-text-main" title={detail.providerKeyName || detail.connectionId}>
                      <div className="truncate">{detail.providerKeyName || "—"}</div>
                      {detail.providerKeyMasked && <div className="truncate font-mono text-xs text-text-muted">{detail.providerKeyMasked}</div>}
                    </td>
        ...
        {selectedDetail && (
          <div className="space-y-6">
            <div className="grid min-w-0 grid-cols-1 gap-4 text-sm sm:grid-cols-2">
              ...
              <div>
                <span className="text-text-muted">Provider API Key:</span>{" "}
                <span className="text-text-main font-medium">{selectedDetail.providerKeyName || "—"}</span>
                {selectedDetail.providerKeyMasked && (
                  <span className="ml-2 font-mono text-text-muted">{selectedDetail.providerKeyMasked}</span>
                )}
              </div>
              <div>
            ...
            <div className="space-y-4">
              <CollapsibleSection title="1. Request Configuration" defaultOpen={true} icon="tune">
                <pre className="max-h-[300px] max-w-full overflow-auto rounded-lg border border-black/5 bg-black/5 p-3 font-mono text-xs text-text-main dark:border-white/5 dark:bg-white/5 sm:p-4">
                  {JSON.stringify(selectedDetail.requestFlow, null, 2)}
                </pre>
              </CollapsibleSection>

              {selectedDetail.responseConfig && (
                <CollapsibleSection title="2. Response Evidence" icon="fact_check">
                  <pre className="max-h-[300px] max-w-full overflow-auto rounded-lg border border-black/5 bg-black/5 p-3 font-mono text-xs text-text-main dark:border-white/5 dark:bg-white/5 sm:p-4">
                    {JSON.stringify({ ...selectedDetail.responseConfig, tokens: selectedDetail.tokens, latency: selectedDetail.latency }, null, 2)}
                  </pre>
                </CollapsibleSection>
              )}
            </div>
